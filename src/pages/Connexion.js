...
import logo from '../images/bateau_3.png';
    ...
    <div className='fond-secondaire-connexion'>
      <header>
        <nav>
          ...
          {!auth && <Link to="/connexion" style={{color: '#F9943B'}}>Connexion</Link>}
      ...
      <main>
        <div className="container-connexion">
          <div className="left-side-connexion">
            ...
            <div>
              <button className="btn-co-ins-connexion" onClick={handleLogin}>Connexion</button>
            </div>
            {loginError && <div style={{ color: 'red', fontSize:'25px',marginTop: '10px' }}>{loginError}</div>}
            <div>
              <Link onClick={handleForgotPasswordClick} className="forgot-password-connexion">Mot de passe oublié?</Link>
            </div>
          </div>
          ...
          <div className="right-side-connexion">
            ...
            <div>
              <button className="btn-co-ins-connexion" onClick={handleRegister}>S'inscrire</button>
            </div>
            {registerError && <div style={{ color: 'red', fontSize:'25px',marginTop: '10px' }}>{registerError}</div>}
          </div>
        </div>
      </main>
      ...
      {showForgotPasswordPopup && (
        <div className="popup-overlay">
          <div className="popup-content">
            ...
            <div><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Votre email" className="input-mail" /></div>
            <button className="btn-co-ins-connexion" onClick={handleForgottenEmail}>Envoyer</button>
            <button className="btn-close-popup" onClick={handleClosePopup}>Fermer</button>
          </div>
        </div>
      )}
      <footer>
