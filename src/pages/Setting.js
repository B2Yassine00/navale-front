...
import { AudioContext } from '../AudioContext'; // Import du contexte audio global
import customBoat from '../images/bateau_1.png';
import classicBoat from '../images/bateau_2.png';
import { default as logo, default as pirateBoat } from '../images/bateau_3.png';
import settingsicon from '../images/settings_icon.png';
import userco from '../images/user_co.png';
import './Setting.css';
...
const Setting = () => {
  ...
  const { isPlaying, togglePlayPause, switchAudio, activeAudio, volume, setVolume } = useContext(AudioContext);
  ...
  const handleValidation = () => {
    const flagMessage = flag ? 'Drapeau personnalisé ajouté' : 'Pas de drapeau personnalisé';
    ...
    <div className="page-setting">
      <header>
        <nav>
          ...
          <Link to="/connexion/profile"><img src={userco} alt="user-co" className="user-co" /></Link>
          <Link to="/connexion/setting"><img src={settingsicon} alt="Settingscon" className="settings-icon" /></Link>
        </nav>
      </header>
      <main>
        <div className="content-setting-single-section">
          <section className="setting-section">
            <div className="select-container">
              ...
              <div className="boat-selection">
                <img
                  ...
                  className={`boat-image ${boatDesign === 'classic' ? 'selected' : ''}`}
                  onClick={() => setBoatDesign('classic')}
                />
                <img
                  src={pirateBoat}
                  alt="Bateau Pirate"
                  className={`boat-image ${boatDesign === 'pirate' ? 'selected' : ''}`}
                  onClick={() => setBoatDesign('pirate')}
                />
                <img
                  src={customBoat}
                  alt="Bateau Personnalisé"
                  className={`boat-image-3 ${boatDesign === 'custom' ? 'selected' : ''}`}
            ...
            <div className="select-container">
              ...
              <select className='input-setting-choice' value={difficulty} onChange={(e) => setDifficulty(e.target.value)}>
            ...
            <div className="input-container">
              ...
              {flag && <img src={flag}  alt="Drapeau personnalisé" className="flag-preview" />}
            </div>

            <div className="select-container">
              <h3>Musique de fond</h3>
             
              <select className='input-setting-choice' value={activeAudio} onChange={handleMusicChange}>
